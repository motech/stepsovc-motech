...
public class StepsovcSmsServiceImpl implements SmsService {
    ...
    private static final Logger log = Logger.getLogger(StepsovcSmsServiceImpl.class);
    ...
    @Autowired
    public StepsovcSmsServiceImpl(MotechSchedulerService motechSchedulerService, MessageSplitter messageSplitter, Properties smsApiProperties) {
    ...
    private void scheduleOrRaiseSendSmsEvent(final List<String> recipients, final String message, final DateTime deliveryTime) {
        List<String> partMessages = Arrays.asList(message);//messageSplitter.split(message, PART_MESSAGE_SIZE, PART_MESSAGE_HEADER_TEMPLATE, PART_MESSAGE_FOOTER);
